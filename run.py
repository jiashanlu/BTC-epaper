...
@routes.get("/get/display.bmp")
async def serve_image(request):
    status = request.app["context"].get_status("home")
    if status is None:
    ...
    output = io.BytesIO()
    status["image"].save(output, format="bmp", bits=8, compress_level=2)
    output.seek(0, 0)
...
async def initialize_displays(context, config_path):
    ...
    for id, display_config in config["displays"].items():
        ...
        asyncio.create_task(display_updater(id, display, config))
...
@click.option("--port", default=8081, help="The port to listen to")
